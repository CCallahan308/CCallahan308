...
**Business Intelligence · Analytics · Dashboard Design — the BI function small organizations can't staff. Founder & Principal at [CGC Labs](https://cgclabs.org).**

I spent four years as the business intelligence function inside a rural critical access hospital: 200+ automated reports and regulatory submissions with zero missed deadlines, a 22-point patient-satisfaction lift, a first-ever 75th-percentile HCAHPS ranking, and a $150,000 vendor dashboard replaced with something people actually opened. Now I run [CGC Labs](https://cgclabs.org), the outsourced BI and analytics function for hospitals that can't staff one.

What I build:

- **Business intelligence** — one agreed definition per metric, so finance and quality walk into the same meeting with the same number, and driver analysis that finds the lever that moves the score
- **Dashboards and reporting** — designed around how the organization runs, not how a vendor assumed it would
- **The analytics underneath** — forecasting, experimentation, churn/LTV modeling with the evaluation discipline to know when a model isn't working: leakage-safe labeling, real baselines, calibration, and a clear line between synthetic and real data. Every metric is committed and reproducible from a clean clone.

**I optimize for decisions over dashboards, and correct methodology over leaderboard numbers.**
...
`SQL` `Tableau` `Python` `TypeScript` `DuckDB` `dbt` `PostgreSQL` `pandas` `scikit-learn` `XGBoost` `LightGBM` `Optuna` `statsmodels` `Streamlit` `FastAPI` `Next.js` `Prisma` `pytest` `GitHub Actions` `Docker`
